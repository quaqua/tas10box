class QueryScriptsController < Tas10boxController
  ...
  def index
    ...
    @query_scripts = @query_scripts.not.where(:query => nil) if params[:query] && params[:query] == "true"
  ...
  def show
    @query_script = QueryScript.where(:id => params[:id]).first_with_user( current_user )
