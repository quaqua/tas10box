    ...
    if @doc.can_share?
      if @user = get_user_by_id
        if @doc.share( @user, params[:privileges] ) && @doc.save( :safe => true )
          ...
          Tas10::AuditLog.create!( :user => current_user, :doc_name => @user.fullname_or_name, :doc_type => @user.class.name, :action => 'audit.shared' )
    ...
    if @doc.can_share?
      if @user = get_user_by_id
        if @doc.unshare( @user ) && @doc.save( :safe => true )
          Tas10::AuditLog.create!( :user => current_user, :doc_name => @user.fullname_or_name, :doc_type => @user.class.name, :action => 'audit.unshared' )
